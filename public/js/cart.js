...
async function addToCart(product) {
    ...
    const safeStock = window.productData?.stocksPorColor?.[color] || stock || 1;
    const safeQty = Math.min(parseInt(cantidad), safeStock);

    const cartItem = {
        producto_id: id, nombre, precio, cantidad: safeQty, colorSeleccionado: color,
        descuento, stock: safeStock, imagen};
    ...
    try {
        ...
        if (!authenticated) {
            ...
            existing ? existing.cantidad = Math.min(existing.cantidad + safeQty, safeStock) 
